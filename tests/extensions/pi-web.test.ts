...
// ── automatic input-derived tab titles ──────────────────────────────
describe('automatic input-derived tab titles', () => {
  const orig = { ...process.env };
  ...
  it('sets an input-derived title by default', async () => {
    delete process.env.PI_WEB_DISABLE_INPUT_DERIVED_TITLE;
    const { pi, ctx, handlers } = createExtensionHarness();
    const inputHandler = handlers.get('input')?.[0];

    expect(inputHandler).toBeDefined();
    const result = await inputHandler({ text: 'fix the dashboard widget bug' }, ctx);

    expect(ctx.ui.setTitle).toHaveBeenCalledWith('Fix Dashboard Widget Bug');
    expect(pi.setSessionName).toHaveBeenCalledWith('Fix Dashboard Widget Bug');
    expect(result).toEqual({ action: 'continue' });
  });

  it('skips input-derived titles when PI_WEB_DISABLE_INPUT_DERIVED_TITLE is set', async () => {
    process.env.PI_WEB_DISABLE_INPUT_DERIVED_TITLE = '1';
    const { pi, ctx, handlers } = createExtensionHarness();
    const inputHandler = handlers.get('input')?.[0];
